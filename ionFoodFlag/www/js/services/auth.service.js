...
angular.module('ionFoodFlagApp')
  ...
  .factory('Auth', function (FURL, $firebaseAuth, $firebaseObject, $firebaseArray, $state, toaster) {
    ...
    // Provide an object for the authorization methods
    var auth = $firebaseAuth(ref);
    // Make a sync object with all of the users as an array
    var users = $firebaseArray(new Firebase(FURL + "profile/"));
    ...
    var authError = {
      AUTH_MANAGER: 10000 + roles.MANAGER,
      AUTH_ADMIN: 10000 + roles.ADMIN
    };

    // Determine the platform that we are running on
    var isApple = false;
    ...
    var Auth = {
      ...
      user: {},
      // Provide a link for the user profiles sync object
      all: users,
      // Provide a temporary user object
      tmpUser: null,

      // Disable social logins for apple, since it doesn't work right now
      isApple: isApple,
      ...
      // isManager - is the user an manager
      isManager: function () {
        // Is someone logged in?
        if (Auth.signedIn()) {
          // Is the logged in user an MANAGER or an ADMIN
          if (Auth.user.profile.role === roles.MANAGER || Auth.user.profile.role === roles.ADMIN) {
            // return true only if logged in user is a manager or admin
            return true;
          }
        }
        // If not a manager or admin, return false
        return false;
      },
      ...
      isAdmin: function () {
        ...
        if (Auth.signedIn()) {
          ...
          if (Auth.user.profile.role === roles.ADMIN) {
            // return true only if logged in user is an admin
            return true;
          }
        }
